{{-- ============================================================ --}}
{{-- 9. dokter/trash.blade.php --}}
{{-- ============================================================ --}}

@extends('layouts.lte.app')

@section('title', 'Trash - Dokter')
@section('page', 'Trash Dokter')
...
<div class="max-w-7xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
        <div class="bg-gradient-to-r from-red-700 to-red-600 px-6 py-4">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-red-100 text-sm mt-1">Data dokter yang telah dihapus sementara</p>
                </div>
                <div class="flex items-center space-x-4">
                    <span class="text-red-100">Total: {{ $dokters->count() }} data</span>
                ...
                <tbody>
                    ...
                    <tr class="border-b border-gray-200 hover:bg-red-50 transition duration-150">
                        ...
                        <td class="p-4">
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                <i class="fas fa-user-md mr-1"></i>{{ $dokter->nama_role ?? 'Dokter' }}
                            </span>
                        </td>
                        <td class="p-4">
                            @if($dokter->status == 1)
                        ...
                        <td class="p-4 text-gray-600">
                            <div class="flex flex-col">
                                <span class="text-sm">{{ \Carbon\Carbon::parse($dokter->deleted_at)->format('d M Y') }}</span>
                                <span class="text-xs text-gray-500">{{ \Carbon\Carbon::parse($dokter->deleted_at)->format('H:i:s') }}</span>
                            </div>
                        </td>
                        <td class="p-4">
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
                                <i class="fas fa-user mr-1"></i>{{ $dokter->deleted_by_name ?? 'System' }}
                            </span>
                        </td>
                        <td class="p-4">
                            <div class="flex gap-2">
                                <form action="{{ route('admin.dokter.restore', $dokter->idrole_user) }}"
                                      method="POST"
                                      onsubmit="return confirm('Apakah Anda yakin ingin mengembalikan dokter ini?')">
                                    @csrf
                                    <button type="submit"
                                ...
                                <form action="{{ route('admin.dokter.force-delete', $dokter->idrole_user) }}"
                                      method="POST"
                    ...
                    <tr>
                        <td colspan="9" class="p-8 text-center text-gray-500">
                            ...
                            <p class="text-sm text-gray-400 mt-2">Tidak ada data dokter yang dihapus</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
        ...
        <div class="p-4 bg-gray-50 border-t border-gray-200 text-sm text-gray-700">
            Menampilkan <span class="font-medium">{{ $dokters->count() }}</span> data di trash
        </div>
        @endif
    </div>
</div>
