{{-- ============================================================ --}}
{{-- 5. role/trash.blade.php --}}
{{-- ============================================================ --}}

@extends('layouts.lte.app')

@section('title', 'Trash - Role')
@section('page', 'Trash Role')
...
<div class="max-w-7xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
        <div class="bg-gradient-to-r from-red-700 to-red-600 px-6 py-4">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-red-100 text-sm mt-1">Data role yang telah dihapus sementara</p>
                </div>
                <div class="flex items-center space-x-4">
                    <span class="text-red-100">Total: {{ $roles->count() }} data</span>
                ...
                <tbody>
                    ...
                    <tr class="border-b border-gray-200 hover:bg-red-50 transition duration-150">
                        ...
                        <td class="p-4">
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                                <i class="fas fa-user-tag mr-1"></i>{{ $item->nama_role }}
                            </span>
                        </td>
                        ...
                        <td class="p-4">
                            <div class="flex gap-2">
                                <form action="{{ route('admin.role.restore', $item->idrole) }}"
                                      method="POST"
                                      onsubmit="return confirm('Apakah Anda yakin ingin mengembalikan role ini?')">
                                    @csrf
                                    <button type="submit"
                                ...
                                <form action="{{ route('admin.role.force-delete', $item->idrole) }}"
                                      method="POST"
                    ...
                    <tr>
                        <td colspan="6" class="p-8 text-center text-gray-500">
                            ...
                            <p class="text-sm text-gray-400 mt-2">Tidak ada data role yang dihapus</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
        ...
        <div class="p-4 bg-gray-50 border-t border-gray-200 text-sm text-gray-700">
            Menampilkan <span class="font-medium">{{ $roles->count() }}</span> data di trash
        </div>
        @endif
    </div>
</div>
