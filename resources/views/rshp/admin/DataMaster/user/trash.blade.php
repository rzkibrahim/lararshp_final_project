{{-- ============================================================ --}}
{{-- 6. user/trash.blade.php --}}
{{-- ============================================================ --}}

@extends('layouts.lte.app')

@section('title', 'Trash - User')
@section('page', 'Trash User')
...
<div class="max-w-7xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
        <div class="bg-gradient-to-r from-red-700 to-red-600 px-6 py-4">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-red-100 text-sm mt-1">Data user yang telah dihapus sementara</p>
                </div>
                <div class="flex items-center space-x-4">
                    <span class="text-red-100">Total: {{ $users->count() }} data</span>
                ...
                <tbody>
                    ...
                    <tr class="border-b border-gray-200 hover:bg-red-50 transition duration-150">
                        ...
                        <td class="p-4 text-gray-600">
                            <div class="flex flex-col">
                                <span class="text-sm">{{ \Carbon\Carbon::parse($user->deleted_at)->format('d M Y') }}</span>
                                <span class="text-xs text-gray-500">{{ \Carbon\Carbon::parse($user->deleted_at)->format('H:i:s') }}</span>
                            </div>
                        </td>
                        <td class="p-4">
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
                                <i class="fas fa-user mr-1"></i>{{ $user->deleted_by_name ?? 'System' }}
                            </span>
                        </td>
                        <td class="p-4">
                            <div class="flex gap-2">
                                <form action="{{ route('admin.user.restore', $user->iduser) }}"
                                      method="POST"
                                      onsubmit="return confirm('Apakah Anda yakin ingin mengembalikan user ini?')">
                                    @csrf
                                    <button type="submit"
                    ...
                    <tr>
                        <td colspan="7" class="p-8 text-center text-gray-500">
                            ...
                            <p class="text-sm text-gray-400 mt-2">Tidak ada data user yang dihapus</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
        ...
        <div class="p-4 bg-gray-50 border-t border-gray-200 text-sm text-gray-700">
            Menampilkan <span class="font-medium">{{ $users->count() }}</span> data di trash
        </div>
        @endif
    </div>
</div>
